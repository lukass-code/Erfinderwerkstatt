...
#define DELAY_INTERVAL 300  // 250ms pause between each pixel
...
void loop() {
  ...
  for (int pixel = 0; pixel < NUM_PIXELS; pixel++) {           // for each pixe
    int colors_hsv = pixel *(65535 / (NUM_PIXELS));
    NeoPixel.setPixelColor(pixel, NeoPixel.ColorHSV(colors_hsv,255,100));  // it only takes effect if pixels.show() is called
    NeoPixel.show();                                           // send the updated pixel colors to the NeoPixel hardware.
    delay(500);
  }
